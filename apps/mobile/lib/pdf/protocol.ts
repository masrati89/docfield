...
function checklistItemHtml(item: PdfChecklistItem): string {
  ...
  return `
    <div style="display:flex;align-items:center;gap:4px;padding:1.5px 0;font-size:10px;color:${PDF.md};line-height:1.3;">
      ...
      ${item.status !== 'ok' && item.refNumber ? `<span style="color:${PDF.red};font-size:8px;flex-shrink:0;">\u05E8\u05D0\u05D4 \u05E1\u05E2\u05D9\u05E3 #${item.refNumber}</span>` : ''}
    </div>`;
}
  ...
  let html = `
    <div style="padding:4px 0 5px;border-bottom:0.5px solid ${PDF.bdrLt};">
      <div style="display:flex;align-items:flex-start;gap:6px;">
        <div style="font-size:10px;font-weight:600;color:${PDF.md};width:16px;text-align:center;flex-shrink:0;margin-top:1px;">${num}.</div>
        <div style="flex:1;">
          <div style="font-size:11px;font-weight:500;color:${PDF.dk};line-height:1.4;">${escapeHtml(text)}</div>
          <div style="font-size:9px;color:${PDF.lt};margin-top:1px;">${escapeHtml(location)}</div>
        </div>
      </div>`;
...
export function generateProtocolHtml(data: PdfReportData): string {
  ...
  function _colHtml(entries: [string, PdfChecklistItem[]][]): string {
    ...
    for (let idx = 0; idx < entries.length; idx++) {
      const [cat, items] = entries[idx];
      html += `<div style="font-size:9px;font-weight:700;color:${PDF.dk};border-bottom:0.5px solid ${PDF.bdr};padding-bottom:2px;margin-bottom:2px;${idx > 0 ? 'margin-top:4px;' : ''}">${escapeHtml(cat)}</div>`;
  ...
  const page1 = `
    <div class="page" style="display:flex;flex-direction:column;">
      ...
      <div style="border-bottom:2px solid ${PDF.dk};padding-bottom:6px;display:flex;justify-content:space-between;align-items:flex-end;">
        <div>
          <div style="font-size:16px;font-weight:700;color:${PDF.dk};letter-spacing:-0.3px;">${title}</div>
          <div style="font-size:10px;color:${PDF.lt};margin-top:2px;font-weight:400;">\u05DE\u05E1\u05D9\u05E8\u05D4 \u05E8\u05D0\u05E9\u05D5\u05E0\u05D9\u05EA | \u05EA\u05D0\u05E8\u05D9\u05DA: ${escapeHtml(formatDate(data.reportDate))} | \u05DE\u05E1\u05E4\u05E8 \u05D3\u05D5\u05D7: ${escapeHtml(data.reportNumber)}</div>
        </div>
        ${logoHtml(data.logoUrl)}
      </div>

      <!-- Details -->
      <div style="margin-top:4px;padding:4px 8px;border:1px solid ${PDF.bdr};border-radius:2px;font-size:10px;display:flex;flex-wrap:wrap;gap:2px 14px;background:${PDF.bg};">
      ...
      <div style="border:1px solid ${PDF.bdr};border-radius:2px;padding:6px 8px;background:${PDF.bg};">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 20px;">
          <div>${col1
            .map(([cat, items]) => {
              let s = `<div style="font-size:9px;font-weight:700;color:${PDF.dk};border-bottom:0.5px solid ${PDF.bdr};padding-bottom:2px;margin-bottom:2px;">${escapeHtml(cat)}</div>`;
        ...
        <!-- Legend -->
        <div style="margin-top:4px;padding-top:3px;border-top:0.5px solid ${PDF.bdr};display:flex;justify-content:space-between;align-items:center;font-size:8px;color:${PDF.lt};">
      ...
      ${footerHtml(1, title, formatDate(data.reportDate), data.logoUrl)}
    </div>`;
          ...
          continuationPages.push(`
            <div class="page" style="display:flex;flex-direction:column;">
              <div style="display:flex;justify-content:space-between;align-items:center;padding-bottom:4px;border-bottom:1px solid ${PDF.bdr};margin-bottom:4px;">
                <div style="font-size:12px;font-weight:600;color:${PDF.dk};">${title} | ${escapeHtml(data.property.projectName)} | \u05D3\u05D9\u05E8\u05D4 ${escapeHtml(data.property.apartmentNumber)}</div>
                ${logoHtml(data.logoUrl)}
              </div>
              ${sectionTitle('\u05DE\u05DE\u05E6\u05D0\u05D9\u05DD (\u05D4\u05DE\u05E9\u05DA)')}
              ${currentPageContent}
              ${footerHtml(pageNum, title, formatDate(data.reportDate), data.logoUrl)}
            </div>`);
          currentPageContent = '';
          defectsOnPage = 0;
          pageNum++;
    ...
    <div class="page" style="display:flex;flex-direction:column;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:5px;margin:3px 0 6px;">
        <div style="text-align:center;padding:5px 3px;background:${PDF.bg};border-radius:3px;border:1px solid ${PDF.bdrLt};">
          <div style="font-size:18px;font-weight:700;color:${PDF.dk};">${clTotal}</div>
          <div style="font-size:8px;color:${PDF.lt};margin-top:1px;">\u05E4\u05E8\u05D9\u05D8\u05D9\u05DD</div>
        </div>
        <div style="text-align:center;padding:5px 3px;background:#ecfdf5;border-radius:3px;border:1px solid ${PDF.bdrLt};">
          <div style="font-size:18px;font-weight:700;color:${PDF.dk};">${clOk}</div>
          <div style="font-size:8px;color:${PDF.lt};margin-top:1px;">\u05EA\u05E7\u05D9\u05DF</div>
        </div>
        <div style="text-align:center;padding:5px 3px;background:#fef2f2;border-radius:3px;border:1px solid ${PDF.bdrLt};">
          <div style="font-size:18px;font-weight:700;color:${PDF.dk};">${clDefect}</div>
          <div style="font-size:8px;color:${PDF.lt};margin-top:1px;">\u05DC\u05D0 \u05EA\u05E7\u05D9\u05DF</div>
        </div>
        <div style="text-align:center;padding:5px 3px;background:#fefaed;border-radius:3px;border:1px solid ${PDF.bdrLt};">
          <div style="font-size:18px;font-weight:700;color:${PDF.dk};">${clPartial}</div>
          <div style="font-size:8px;color:${PDF.lt};margin-top:1px;">\u05D7\u05DC\u05E7\u05D9</div>
        </div>
      </div>
      ...
      ${
        ...
        <div style="padding:4px 8px;border:1px solid ${PDF.bdr};border-radius:2px;background:${PDF.bg};font-size:10px;">
      ...
      ${
        ...
        <div style="padding:5px 8px;border:1px solid ${PDF.bdr};border-radius:2px;background:${PDF.bg};min-height:24px;font-size:10px;color:${PDF.md};line-height:1.5;">${escapeHtml(data.tenantNotes)}</div>
      `
          : ''
      }

      ${
        data.showProtocolTerms !== false
          ? `
      ${sectionTitle('\u05EA\u05E0\u05D0\u05D9\u05DD \u05D5\u05D0\u05D7\u05E8\u05D9\u05D5\u05EA')}
      <div style="font-size:8px;color:${PDF.lt};line-height:1.5;padding:2px 0;">
        ${escapeHtml(data.protocolTerms ?? REPORT_DEFAULTS.protocol_terms)}
      </div>`
          : ''
      }
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:3px;">
        ${signatureBoxHtml(
          ...
          escapeHtml(formatDate(data.reportDate)),
      ...
      ${
        ...
        <div style="margin-top:10px;text-align:center;">
          <div style="display:inline-block;padding:5px 18px;border:1px solid ${PDF.bdr};border-radius:3px;">
            <img src="${escapeAttr(data.stampUrl)}" style="max-height:24px;object-fit:contain;" />
          </div>
        </div>
      `
          : ''
