  ...
  let html = `
    <div style="padding:4px 0 5px;border-bottom:0.5px solid ${PDF.bdrLt};">
      <div style="display:flex;align-items:flex-start;gap:6px;">
        ...
        <div style="flex:1;">
          <div style="font-size:8.5px;font-weight:500;color:${PDF.dk};line-height:1.4;">${text}</div>
          <div style="font-size:7px;color:${PDF.lt};margin-top:1px;">${location}</div>
        </div>
      </div>`;
...
export function generateProtocolHtml(data: PdfReportData): string {
  ...
  function _colHtml(entries: [string, PdfChecklistItem[]][]): string {
    ...
    for (const [cat, items] of entries) {
      html += `<div style="font-size:7px;font-weight:700;color:${PDF.dk};border-bottom:0.5px solid ${PDF.bdr};padding-bottom:2px;margin-bottom:2px;${entries.indexOf([cat, items]) > 0 ? 'margin-top:4px;' : ''}">${cat}</div>`;
  ...
  for (const group of groups) {
    ...
    for (const defect of group.defects) {
      ...
      if (!groupHasItems) {
        page1Defects += subSectionTitle(group.category);
        groupHasItems = true;
      }
    ...
    <div class="page" style="display:flex;flex-direction:column;">
      ...
      <div style="border-bottom:2px solid ${PDF.dk};padding-bottom:6px;display:flex;justify-content:space-between;align-items:flex-end;">
        <div>
          ...
          <div style="font-size:7.5px;color:${PDF.lt};margin-top:2px;font-weight:400;">\u05DE\u05E1\u05D9\u05E8\u05D4 \u05E8\u05D0\u05E9\u05D5\u05E0\u05D9\u05EA | \u05EA\u05D0\u05E8\u05D9\u05DA: ${data.reportDate} | \u05DE\u05E1\u05E4\u05E8 \u05D3\u05D5\u05D7: ${data.reportNumber}</div>
        </div>
      ...
      <div style="margin-top:4px;padding:4px 8px;border:1px solid ${PDF.bdr};border-radius:2px;font-size:7.5px;display:flex;flex-wrap:wrap;gap:2px 14px;background:${PDF.bg};">
        ...
        ${detailRow('\u05D3\u05D9\u05D9\u05E8:', `${data.client.name}${data.client.idNumber ? ` | \u05EA.\u05D6. ${data.client.idNumber}` : ''}`)}
      ...
      <div style="border:1px solid ${PDF.bdr};border-radius:2px;padding:6px 8px;background:${PDF.bg};">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0 20px;">
          <div>${col1
            .map(([cat, items]) => {
              let s = `<div style="font-size:7px;font-weight:700;color:${PDF.dk};border-bottom:0.5px solid ${PDF.bdr};padding-bottom:2px;margin-bottom:2px;">${cat}</div>`;
  ...
  if (remainingDefects.length > 0) {
    ...
    for (const group of remainingGroups) {
      currentPageContent += subSectionTitle(group.category);
      for (const defect of group.defects) {
          ...
          continuationPages.push(`
            <div class="page" style="display:flex;flex-direction:column;">
              <div style="display:flex;justify-content:space-between;align-items:center;padding-bottom:4px;border-bottom:1px solid ${PDF.bdr};margin-bottom:4px;">
                <div style="font-size:9px;font-weight:600;color:${PDF.dk};">${title} | ${data.property.projectName} | \u05D3\u05D9\u05E8\u05D4 ${data.property.apartmentNumber}</div>
                ${logoHtml(data.logoUrl)}
              </div>
  ...
  const lastPage = `
    <div class="page" style="display:flex;flex-direction:column;">
      ...
      ${
        ...
        <div style="padding:4px 8px;border:1px solid ${PDF.bdr};border-radius:2px;background:${PDF.bg};font-size:8px;">
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1px 10px;">
            ${data.keyDelivery.map((k) => `<div style="display:flex;gap:3px;"><span style="color:${PDF.lt};">${k.label}:</span><span style="color:${PDF.dk};font-weight:600;">${k.value}</span></div>`).join('')}
          </div>
        </div>
      `
          : ''
      ...
      ${
        ...
        <div style="padding:5px 8px;border:1px solid ${PDF.bdr};border-radius:2px;background:${PDF.bg};min-height:24px;font-size:8px;color:${PDF.md};line-height:1.5;">${data.tenantNotes}</div>
      `
          : ''
      }
